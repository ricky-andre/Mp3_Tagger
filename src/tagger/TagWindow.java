...
public class TagWindow extends JFrame
{
    ...
    // create a structure that contains the possible genres in a hash, the genre in a
    ...
    // fields names that can be used to set fields. The spelling is important since they are used
    // to set the fields using the Mp3info class, using the command set(field_name,field_value).
    // They are also case sensitive!
    private String tagv1fieldsnames[]=new String [] {"artist","title","album","year","genre","comment","track"};
    ...
    // this function takes a hash and a myfile object and sets all the fields in the hash
    // than it checks for unsupported fields (version 2.4) and asks if conversion is wanted
	...
	public void createInterface ()
	{
	    ...
	    String tooltips[]=new String [] {"move up selected rows","move down selected rows","add field of no interest","remove selected rows"};
	    MyButton button;
	    ...
	    tmp2.setLayout(new BoxLayout(tmp2,BoxLayout.X_AXIS));
	    tmp2.setAlignmentY(Component.TOP_ALIGNMENT);
	    border=BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),"Expected file name format",TitledBorder.LEFT,TitledBorder.TOP);
	    ...
	    tooltips=new String [] {"remove from table the files you've just successfully tagged","try to perform tag by name operation","execute tag by name operation"};
	    for (int i=0;i<butstr.length;i++)
		{
		    button=new MyButton(butstr[i],Utils.getImage("tagbyname",iconsid[i]),this);
		    button.setActionCommand(commands[i]);
	    ...
	    warningScrollPane=new JScrollPane(warningArea,ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	    ...
	    progressManager(18);
	    //System.out.println("tagbyname %"+(System.currentTimeMillis()-window.time)*100/20x7fffffff);
	}
	    ...
	    if (command.equals("try") || command.equals("execute"))
		{
		    ...
		    if (dotry)
			{
			    ...
			    else
				{
				    ...
				    JOptionPane.showMessageDialog(null,"Another process is active!","Another process is active!",JOptionPane.INFORMATION_MESSAGE);
				}
			}
		}
				    ...
				    for (int k=0;k<values.length;k++)
					{
					    if ((values[k][0]).equals(tablecolumns[j]))
						{
						    ...
						    else
							{
								...
								{
								    fieldseperr.append(", field <font color=blue>\""+values[k][0]+"\"</font> contains separator");
								}
							}
						    table.repaint();
						}
		    ...
		    if (error)
			{
			    ...
			    // warningArea.addline(WarnPanel.OK);
                            if (fieldseperr.length()!=0)
                                warningArea.addline(WarnPanel.WARNING,"<html><font color=black size=-1><B>File <font color=blue>\""+selFiles[i].getName()+"\"</font>"+fieldseperr.toString()+", possible configuration error");
                        }
		}  // if update
	    else
		{
		...
		if (config.optionwincfg.safewritev1)
			...
			for (int i=0;i<tot;i++)
				...
				if (writev2)
					...
					String fields[]=new String[] {"artist","title","genre","album","track","comment","year"};
	...
	public void createInterface ()
	{
	    ...
	    String tooltips[]=new String [] {"move up selected rows","move down selected rows","remove selected rows"};
	    MyButton button;
	    for (int i=0;i<butstr.length;i++)
		{
	    ...
	    tmp.setLayout (new BoxLayout(tmp,BoxLayout.X_AXIS));
	    tmp.setAlignmentY(Component.TOP_ALIGNMENT);
	    border=BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),"Output file name format",TitledBorder.LEFT,TitledBorder.TOP);
	    ...
	    commands=new String [] {"remove","try","execute"};
            iconsid=new String [] {"removesuccess","try","execute"};
	    tooltips=new String [] {"remove from table the files you have successfully renamed","try to perform rename by tag operation","execute rename by tag operation"};
	    for (int i=0;i<butstr.length;i++)
		{
		    button=new MyButton(butstr[i],Utils.getImage("renamebytag",iconsid[i]),this);
		    button.setActionCommand(commands[i]);
	    ...
	    //System.out.println("rename by tag %"+(System.currentTimeMillis()-window.time)*100/20x7fffffff);
	}
	    ...
	    if (update)
		{
		    ...
		    if (!error)
			{
			    ...
			    if (command.equals("execute"))
				{
				    ...
				    filepath=new String(filepath.substring(0,filepath.length()-selFiles[i].getName().length())+renamed);
				    ...
				    else
					{
					    ...
					    error=true;
                                            warningArea.append(", unexpected error in renaming file (does the new name contain forbidden characters such as \"\\/*?:\" ?)!");
					    table.repaint();
					}
				}
	...
	private IconSelect checkfield[]=new IconSelect[]
	...
	public void createInterface ()
	{
	    ...
	    border=BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),"Tag fields for mass tag operations",TitledBorder.LEFT,TitledBorder.TOP);
	    tmp.setBorder(border);
	    ...
	    for (int i=0;i<fieldstr.length;i++)
		{
			...
			tmp4.add(createField(fieldstr[i],Utils.getImage("masstag",imgnames[i]),Utils.getImage("masstag",imgnames2[i])));
		    ...
		    tmp.add(tmp4);
		    //System.out.println("mass tag field "+fieldstr[i]+" "+(System.currentTimeMillis()-window.time)*100/20x7fffffff);
		}
	    ...
	    tmp.setLayout(new BoxLayout(tmp,BoxLayout.X_AXIS));
	    tmp.setAlignmentX(Component.LEFT_ALIGNMENT);
	    // tmp.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),"Operation and options selection",TitledBorder.LEFT,TitledBorder.TOP));
	    ...
	    String tooltips[]=new String [] {"refresh table","remove from table the files on wich you have successfully operated","try to perform mass tag operation","execute mass tag operation"};
	    for (int i=0;i<butstr.length;i++)
		{
	    ...
	    //System.out.println("mass tag field action panel % "+(System.currentTimeMillis()-window.time)*100/20x7fffffff);
	}
	...
	private boolean checkFieldCorrect (String field,String value)
	{
	    if (field.equals("track"))
		{
			...
			{
			    // warningArea.append("Invalid track field "+fields[i].getText()+", can't write tag");
	...
	private void fixColumns (String command)
	{
	    ...
	    else
		{
		    ...
		    int minimumsize=filenamesize+smallertotalsize+(tablecolumns.length-smallercounter-1)*othercolumnssize;
		    // table.setPreferredScrollableViewportSize(new Dimension(Math.max(size,minimumsize),dim));
		}
	    for (int i=0;i<tablecolumns.length;i++)
		{
		    columns=table.getColumnModel().getColumn(i);
	...
	public void actionPerformed (ActionEvent e)
	{
	    ...
	    else if (command.equals("try") || command.equals("execute"))
		{
		    ...
		    if (dotry)
			{
				...
				{
				    // warning window, waiting task to finish!
				    JOptionPane.showMessageDialog(null,"Another process is active,\nwait for it to finish!","Warning message!",JOptionPane.WARNING_MESSAGE);
				}
			}
		}
	}
	    ...
	    */
	    ...
	    if (update)
		{
		    ...
		    if (!error)
			{
			    ...
			    if (actiontodo.equals("clear tag v1 (some fields)") || actiontodo.equals("clear tags v1-v2 (some fields)"))
				{
				    if (mp3field.id3v1.exists)
					{
			    ...
			    if (actiontodo.equals("clear tag v2 (some fields)") || actiontodo.equals("clear tags v1-v2 (some fields)"))
				{
				    if (mp3field.id3v2.exists)
					{
					    for (int c=0;c<fieldsToSet.length && exec;c++)
				...
				{
				    if (actiontodo.equals("clear tag v1 (all fields)") || actiontodo.equals("clear tags v1-v2 (all fields)"))
					{
					    if (mp3field.id3v1.exists)
						{
						    if (exec)
				    ...
				    if (actiontodo.equals("clear tag v2 (all fields)") || actiontodo.equals("clear tags v1-v2 (all fields)"))
					{
					    if (mp3field.id3v2.exists)
						{
						    if (exec)
	...
	public boolean taskExecute (String processId)
	{
		...
		{
		    warningArea.addline(WarnPanel.OK,"<html><font color=black size=-1><B>Operation succesfull on all files!");
		}
	...
	private String mp3fieldsnames[]=new String[] {"bytes length","song length","version","rate","sync byte","frames","sample rate","emphasys","copyright","original","crc"};
	...
	private String tagv2fieldsnames[]=new String[] {"title","artist","album","year","genre","comment","track"};
	...
	private JPanel mp3panel ()
	{
	    ...
	    titled.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),"Mp3 file info",TitledBorder.LEFT,TitledBorder.TOP));
	    ...
	    // "byte length","song length","version","rate","sync byte","frames","sample rate","emphasys","copyright","original","crc"
	...
	{
	    JPanel titled=new JPanel();
	    titled.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),"Tag version 1",TitledBorder.LEFT,TitledBorder.TOP));
	    titled.setLayout(new BoxLayout(titled,BoxLayout.Y_AXIS));
	    ...
	    tmp.add(tmp2);
	    tmp2=genpanelbutton();
	    button=new MyButton(MyButton.NORMAL_BUTTON,null,"v2->v1",Utils.getImage("edittag","editv2tov1"),this);
	    tmp2.add(button);
	    tmp.add(tmp2);
	    tmp2=genpanelbutton();
	    ...
	    tmp.add(tmp2);
            ...
            tmp.setMaximumSize(tmp.getPreferredSize());
	    //tmp2=genpanelbutton();
	    //button=new MyButton(MyButton.NORMAL_BUTTON,null,"writeall",Utils.getImage("edittag","writev1v2"),this);
	    //tmp2.add(button);
	    //tmp.add(tmp2);
	...
	private JPanel Tagv2panel ()
	{
	    ...
	    JLabel label=null;
	    JPanel titled=new JPanel();
	    titled.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),"Tag version 2",TitledBorder.LEFT,TitledBorder.TOP));
	    titled.setLayout(new BoxLayout(titled,BoxLayout.Y_AXIS));
	    JPanel tmp3=new JPanel();
	    tmp3.setLayout(new BoxLayout(tmp3,BoxLayout.Y_AXIS));
	    ...
	    tmp2.setBorder(BorderFactory.createEmptyBorder(0,0,0,10));
	    button=new MyButton(MyButton.NORMAL_BUTTON,null,"adv panel",Utils.getImage("edittag","advpanel"),this);
	    tmp2.add(button);
	    tmp.add(tmp2);
	    ...
	    // tmp2.setAlignmentX(Component.CENTER_ALIGNMENT);
	    tmp2.setBorder(BorderFactory.createEmptyBorder(0,0,0,10));
	    button=new MyButton(MyButton.NORMAL_BUTTON,null,"v1->v2",Utils.getImage("edittag","editv1tov2"),this);
	    tmp2.add(button);
	    tmp.add(tmp2);
	...
	public void createInterface ()
	{
	    ...
	    mp3panel=mp3panel();
	    tmp.add(mp3panel);
	    //System.out.println("edit mp3 panel % "+(System.currentTimeMillis()-window.time)*100/20x7fffffff);
	    ...
	    // Tagv2panel.setMinimumSize(new Dimension(0,(int)tmp.getPreferredSize().getHeight()));
	    // Tagv2panel.setMaximumSize(new Dimension(0x7fffffff,(int)tmp.getPreferredSize().getHeight()));
	    tmp3.add(Tagv2panel);
	    // JScrollPane mainScrollPane=new JScrollPane(tmp3,JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,JScrollPane.HORIZONTAL_SCROLLBAR_ALWAYS);
	    // mainPanel.add(mainScrollPane);
	    mainPanel.add(tmp3);

	    // edittagSplitPane=new JSplitPane (JSplitPane.VERTICAL_SPLIT,mainScrollPane,fileScrollPane);
	...
	private void fixColumns ()
	{
	    ...
	    int minimumsize=0;
	    TableColumn columns = null;
	    int cols[]=new int[tagv1index,tagv2index,bitrate,samplerate,mp3layer,mp3version];
	...
	public void actionPerformed (ActionEvent e)
	{
		...
		{
		    String command=e.getActionCommand();
		    if ((command.equals("writev1") || command.equals("writeall") || command.equals("writev2")) && !fileelem.canWrite())
		    ...
		    if ((command.equals("writev1") || command.equals("writeall")) && fileelem.canWrite())
			{
				...
				{
				    if (!tagv1fieldsnames[i].equals("genre"))
					mp3elem.id3v1.setElem(tagv1fieldsnames[i],((JTextField)tagv1fields.get(tagv1fieldsnames[i])).getText());
				    else
					{
		    ...
		    if ((command.equals("writev2") || command.equals("writeall")) && fileelem.canWrite())
			{
				...
				{
				    if (!tagv2fieldsnames[i].equals("genre"))
					mp3elem.id3v2.setElem(tagv2fieldsnames[i],((JTextField)tagv2fields.get(tagv2fieldsnames[i])).getText());
				    else
					{
		    ...
		    if (command.equals("v2->v1"))
			{
				...
				{
				    if (!tagv2fieldsnames[i].equals("genre"))
					((JTextField)tagv1fields.get(tagv2fieldsnames[i])).setText(((JTextField)tagv2fields.get(tagv1fieldsnames[i])).getText());
				    else
					{
		    ...
		    else if (command.equals("v1->v2"))
			{
				...
				{
				    if (!tagv1fieldsnames[i].equals("genre"))
					((JTextField)tagv2fields.get(tagv2fieldsnames[i])).setText(((JTextField)tagv1fields.get(tagv2fieldsnames[i])).getText());
				    else
					{
		    ...
		    else if (command.equals("adv panel"))
			{
			    if (advfields==null)
				{
				    advfields=new AdvancedTagWindow(AdvancedTagWindow.EDIT,advfieldsconfig,selFiles[selrows[0]].mp3);
				    advfields.addWindowListener(advwinlistener);
			    ...
			    if (singlerename.getText().length()>0)
				{
				    ...
				    if (selFiles[i].canWrite())
					{
					    ...
					    if (selFiles[i].renameTo(file))
						{
						    ...
						    selFiles[i]=file;
						    /*
						    MyFile lists[][]=new MyFile[][] {tagbyname.selFiles,masstag.selFiles,renamebytag.selFiles};
						    int cols[]=new int[] {tagbyname.getCol("File name"),masstag.getCol("File name"),renamebytag.getCol("File name")};
						    Object data[][][]=new Object [][][] {tagbyname.data,masstag.data,renamebytag.data};
						    MyJTable table[]=new MyJTable[] {tagbyname.table,masstag.table,renamebytag.table};
						...
						JOptionPane.showMessageDialog(null,
									      ...
									      "already exists or contains invalid characters!","Error message",JOptionPane.ERROR_MESSAGE);
					} // fine if can write
				    else
					JOptionPane.showMessageDialog(null,
	    ...
	    if (!isAdjusting)
		{
		    ...
		    if (minIndex>=0 && minIndex<selFiles.length)
			{
			    ...
			    if (mp3elem.isMp3())
				{
				    ...
				    ((JLabel)mp3fields.get("version")).setText(" "+mp3elem.getMpgVersion()+", layer "+mp3elem.getMpgLayer());
				    if (mp3elem.isVbr())
					...
					{
					    if (!tagv1fieldsnames[i].equals("genre"))
						((JTextField)tagv1fields.get(tagv1fieldsnames[i])).setText(mp3elem.id3v1.getElem(tagv1fieldsnames[i]));
					    else
						{
				    ...
				    if (!mp3elem.id3v2.exists)
					Tagv2panel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),"Tag version 2 does not exists",TitledBorder.LEFT,TitledBorder.TOP));
				    else
					Tagv2panel.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),"Tag version 2."+mp3elem.id3v2.version,TitledBorder.LEFT,TitledBorder.TOP));
				    ...
				    for (int i=0;i<tagv2fieldsnames.length;i++)
					{
						...
						{
						    ((JTextField)tagv2fields.get(tagv2fieldsnames[i])).setText(mp3elem.id3v2.getElem("track_asis").getValue());
						}
					    else // if (!tagv2fieldsnames[i].equals("genre"))
						((JTextField)tagv2fields.get(tagv2fieldsnames[i])).setText(mp3elem.id3v2.getElem(tagv2fieldsnames[i]).getValue());
					}
				    if (advfields!=null)
			    ...
			    else // if it not an mp3 ...
				{
                                    ...
                                    if (!selFiles[minIndex].exists())
                                        JOptionPane.showMessageDialog(null,"This file has been moved, renamed or deleted!","Warning message",JOptionPane.WARNING_MESSAGE);
                                    else
				        JOptionPane.showMessageDialog(null,"This is not an mp3 file!","Warning message",JOptionPane.WARNING_MESSAGE);
				}
			}
		}
	}
	...
	if (table!=null)
		...
		table=newTable;
		fileScrollPane=new JScrollPane (table,ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,ScrollPaneConstants.HORIZONTAL_SCROLLBAR_ALWAYS);
	...
	if (createfilepanel!=createFilePanelWindow)
		...
		if (createFilePanelWindow)
		    {
			tagbyname.filewarning=new JSplitPane(JSplitPane.VERTICAL_SPLIT,tagbyname.fileScrollPane,tagbyname.warningScrollPane);
			tagbynameSplitPane.setBottomComponent(tagbyname.filewarning);
			masstag.filewarning=new JSplitPane(JSplitPane.VERTICAL_SPLIT,masstag.fileScrollPane,masstag.warningScrollPane);
                        masstagSplitPane.setBottomComponent(masstag.filewarning);
			renamebytag.filewarning=new JSplitPane(JSplitPane.VERTICAL_SPLIT,renamebytag.fileScrollPane,renamebytag.warningScrollPane);
	...
	// the next initialization instruction MUST be execute strictly in the order below!
	...
	*/
		...
		if (createFilePanelWindow)
		    {
                        ...
                        if (valx!=null) tagbynameSplitPane.setDividerLocation(valx.intValue());
                        valx=config.getConfigInt("3.1.div2");
                        if (valx!=null) tagbyname.filewarning.setDividerLocation(valx.intValue());
                        valx=config.getConfigInt("3.3.div1");
                        if (valx!=null) renamebytagSplitPane.setDividerLocation(valx.intValue());
                        valx=config.getConfigInt("3.3.div2");
                        if (valx!=null) renamebytag.filewarning.setDividerLocation(valx.intValue());
                        valx=config.getConfigInt("3.2.div1");
                        if (valx!=null) masstagSplitPane.setDividerLocation(valx.intValue());
                        valx=config.getConfigInt("3.2.div2");
                        if (valx!=null) masstag.filewarning.setDividerLocation(valx.intValue());
		    }
		else
		    {
                        valx=config.getConfigInt("3.1.div1");
                ...
                if (valx!=null) edittagSplitPane.setDividerLocation(valx.intValue());
	    }
	else
	...
	createFilePanelWindow=config.getConfigBoolean("3.createfilepanel");
        ...
        if (valx!=null)
        {
            try {jtabbed.setSelectedIndex(valx.intValue());}
            catch (Exception e) {};
        }
