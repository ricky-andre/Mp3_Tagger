...
public class DinamicTableModel extends EditableTableModel
{
    ...
    public void addRow (ArrayList tmp)
    {
    ...
    public boolean addRow (int index,ArrayList tmp)
    {
	if (index<data.size())
	    {
		if (tmp.size()<columnsnum)
			...
			try
				...
				rowrange=Utils.split(rows[i],"-");
                                if (rowrange[0].trim().length()==0)
                                    { rowstart=0; rowend=getRowCount()-1;}
                                else
				    {
				...
				for (int j=0;j<columns.length;j++)
					...
					colrange=Utils.split(rows[i],"-");
                                        if (colrange[0].trim().length()==0)
                                          { colstart=0; colend=getColumnCount()-1;}
                                      else
				          {
	...
	if (isTableEditable())
		...
		if (editrow.size()>0)
		    {
			conf.append("rows=");
			Set set=editrow.entrySet();
			Iterator iterator=set.iterator();
			while (iterator.hasNext())
			    {
				Map.Entry elem=(Map.Entry)iterator.next();
				conf.append((String)elem.getKey()+",");
			    }
			conf.append("[sep]");
		    }
		...
		try
			...
			if (values[i].startsWith("rows="))
				...
				for (int j=0;j<cells.length;j++)
					...
					setEditableCell(Integer.valueOf(numbers[0][1]).intValue(),Integer.valueOf(numbers[1][1]).intValue(),true);
				    }
			    }
		    }
		catch (Exception e) {}
