...
public class JdbcDatabase implements DatabaseInterface
{
	...
	try
		...
		return true;
		// Connection conn = DriverManager.getConnection("http://localhost/","root","despagna");
	...
	try
		...
		while (!finished)
		    {
			System.out.println("executing "+"select \"one\" from "+table+" limit "+limitstart+","+ROW_COUNT_LIMIT);
			rowcountresult=statement.executeQuery("select \"one\" from "+table+" limit "+limitstart+","+ROW_COUNT_LIMIT);
    ...
    private TreeMap getallrowHash=new TreeMap();
    ...
    private TreeMap getValueAtHash=new TreeMap();
    private final static int HEAD=0;
    private final static int TAIL=1;

    private void purgeHash (TreeMap tree,int rows,int verse)
    {
	...
	try
		...
		if (ret!=null)
			...
			*/
			if ((row > tableTask.lowindex && row < tableTask.lowindex+QUERY_ROW_LIMIT) && tableTask.lowindex>0)
			    {
				int attempts=0;
				while (ret==null)
				    {
				...
				if (!(row < (int)((float)QUERY_ROW_LIMIT*RELOAD_FRACTION_COEFFICIENT))
				    && (row < tableTask.lowindex + (int)((float)QUERY_ROW_LIMIT*RELOAD_FRACTION_COEFFICIENT)))
				    {
				...
				// probably to separate the case of filter for rowsnumber parameter
					 ...
					 && (row > tableTask.lowindex + QUERY_ROW_LIMIT - (int)((float)QUERY_ROW_LIMIT*RELOAD_FRACTION_COEFFICIENT)))
				    {
					tableTask.lowindex=row-(QUERY_ROW_LIMIT/2);
					if (tableTask.lowindex<1)
					    tableTask.lowindex=1;
    ...
    */
	...
	/*
	  if (!execute("select * from "+table+" order by "+Utils.join(cols,",")+" ASC"))
	    return false;
	*/
	...
	*/
    ...
    {
	private Hashtable donevars=new Hashtable();
