...
public class DatabaseWindow extends JFrame implements ActionListener, ItemListener, TaskExecuter, TableModelListener {
	...
	private void createMenuBar() {
		...
		// CreateMenuItem(menu,ITEM_PLAIN,"import from txt",null,'e',"import a text song
		// list into Database");
		bar.add(menu);
		/*
		 * menu=new JMenu("File");
	...
	DatabaseWindow(MainWindow win, final int windowId) {
		...
		tmp2.setAlignmentX(JComponent.LEFT_ALIGNMENT);
		tmp2.setBorder(BorderFactory.createEmptyBorder(0, 0, 0, 10));
		button = new MyButton(MyButton.NORMAL_BUTTON, null, "savetable", Utils.getImage("databasewindow", "save"),
				this);
		...
		button = new MyButton(MyButton.NORMAL_BUTTON, null, "load Database", Utils.getImage("databasewindow", "load"),
				this);
		...
		addWindowListener(new WindowAdapter() {
			public void windowClosing(WindowEvent e) {
				...
				if ((dbinterface instanceof Database) && tablechanged) {
					...
					int n = JOptionPane.showOptionDialog(myself,
							pr,
				...
				if (close) {
					...
					dispose();
					// System.out.println("Available memory after dispose
					// "+((float)Runtime.getRuntime().freeMemory()/1000000));
		...
		if (src.equals(dbsource)) {
			if (item.equals("SQL Database")) {
				...
				if (tabs != null) {
					...
					JOptionPane.showMessageDialog(null, "An error occurred:\n\n" + JdbcUtils.error, "Error occurred",
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (command.equals("set filter options")) {
			...
			if (dbinterface != null) {
				if (orderchanged) {
					...
					JOptionPane.showMessageDialog(null,
							"No need to reorder, table is already ordered\nwith the selected criteria!",
							"Error occurred", JOptionPane.WARNING_MESSAGE);
			} else
				JOptionPane.showMessageDialog(null, "No Database loaded!", "Error occurred", JOptionPane.ERROR_MESSAGE);
			...
			if (true) {
				...
				JOptionPane.showMessageDialog(null, "Select \"text file\" as Database source!", "Error occurred",
						JOptionPane.ERROR_MESSAGE);
			}
			...
			if (dbinterface != null) {
				...
				if (n == JFileChooser.APPROVE_OPTION) {
					...
					if (file.getName().trim().length() == 0)
						JOptionPane.showMessageDialog(null, "Empty name, cannot export!", "Error occurred",
					...
					/*
					 * String
					 * input=JOptionPane.showInputDialog("Type file name value (without extension):"
					 * );
					 ...
					 * }
					 * else
					 * JOptionPane.showMessageDialog(null,"Empty name, cannot write!"
					 * ,"Error occurred",JOptionPane.ERROR_MESSAGE);
					 */
				}
			} else
				JOptionPane.showMessageDialog(null, "No Database loaded, cannot export!", "Error occurred",
						JOptionPane.WARNING_MESSAGE);
			...
			if (dbinterface == null)
				...
				if (tablechanged) {
					...
					JOptionPane.showMessageDialog(null, "No need to save, Database has not changed!", "Error occurred",
							JOptionPane.WARNING_MESSAGE);
			} else
				JOptionPane.showMessageDialog(null, "Use \"export to file\" option to export an SQL Database!",
						"Error occurred", JOptionPane.ERROR_MESSAGE);
			...
			if (dbinterface == null)
				JOptionPane.showMessageDialog(null, "No Database loaded!", "Error occurred",
						JOptionPane.WARNING_MESSAGE);
			else {
				int rows[] = table.getSelectedRows();
				if (rows.length == 0) {
					JOptionPane.showMessageDialog(null, "No row selected!", "Error occurred",
							JOptionPane.WARNING_MESSAGE);
				} else {
					int min = rows[0];
	...
	private void setOrderConfigString(String str) {
		...
		if (numbers.length != ordercols.length) {
			...
			for (int i = 0; i < ordercols.length; i++) {
				try {
					ordercriteria[i] = Integer.valueOf(numbers[i]).intValue();
				} catch (Exception e) {
					ordercols = new String[0];
					ordercriteria = new int[0];
					break;
				}
			}
		}
	...
	public boolean taskExecute(String processId) {
		if (processId.equals("loaddb")) {
			...
			if (!textDatabase.checkHeader()) {
				taskDone = true;
				JOptionPane.showMessageDialog(null,
						"Error reading Database:\n\n" + textDatabase.getError(), "Error occurred",
						JOptionPane.ERROR_MESSAGE);
				taskActive = false;
				return false;
			}
			...
			try {
				...
				// reorder the database if necessary, the artist must the first order
				// criteria!!!
					...
					for (int i = 0; i < dbsize; i++) {
						...
						if (value.length() == 0) {
							if (!pagesRefs.containsKey("other"))
								pagesRefs.put("other", Integer.valueOf(pagecounter));
							continue;
						}
						...
						if (Character.isLetter(letter)) {
							if (!pagesRefs.containsKey(toinsert)) {
								pagesRefs.put(toinsert, Integer.valueOf(pagecounter));
								// System.out.println("letter "+letter+" row "+i+" art "+value);
							}
						} else if (Character.isDigit(letter)) {
							if (!pagesRefs.containsKey(toinsert))
								pagesRefs.put("0-9", Integer.valueOf(pagecounter));
						} else {
							if (!pagesRefs.containsKey(toinsert))
								pagesRefs.put("other", Integer.valueOf(pagecounter));
						}
				...
				while (iterator.hasNext()) {
					...
					if (!(key.equals("0-9") || key.equals("other"))) {
						...
						if (val == 0)
							...
							otherlinks.append("<a href=\"" + htmlfilename + "_" + val + ".html#letter" + key + "\"> "
									+ key + " </a> \n");
					}
					otherlinks.append("&nbsp;");
				}
				...
				while (counter < dbsize) {
					...
					if (counter / maxRowNumberForPage == 0) {
						buffer.append(
								"<h3 align=left><a href=\"" + htmlfilename + "_" + (counter / maxRowNumberForPage + 1) +
										".html\">next page</a></h3><p><p>");
					} else if (counter / maxRowNumberForPage == 1) {
						buffer.append("<table width=100%><tr><td><h3 align=left><a href=\"" + htmlfilename
								+ ".html\">previous page</a></h3></td>");
						...
						buffer.append("<table width=100%><tr><td><h3 align=left><a href=\"" + htmlfilename + "_"
								+ (counter / maxRowNumberForPage - 1) + ".html\">previous page</a></h3></td>");
						buffer.append("<td><h3 align=right><a href=\"" + htmlfilename + "_" +
								(counter / maxRowNumberForPage + 1) +
								".html\">next page</a></h3></tr></table>");
					}
