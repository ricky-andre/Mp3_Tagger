...
public class MyJTable extends JTable {
	...
	public boolean ensureRowVisible(int row) {
		try {
			...
			// if (portHeight>3*height)
			{
				...
				// System.out.println("nowy "+nowy+" port height "+portHeight+" celltop
				// "+celltop+" cell height "+height+" pos"+position);
				if (position >= 0) {
	...
	public boolean setColumnEditor(int col, Object comp) {
		...
		if (comp instanceof JComboBox)
			tablecolumn.setCellEditor(new DefaultCellEditor((MyCombo) comp));
	...
	public boolean minimizeColumnWidth(int colindex) {
		...
		comp = getDefaultRenderer(model.getColumnClass(colindex)).getTableCellRendererComponent(
				this, colname,
	...
	public String getConfigString() {
		...
		if (model.saveconfig == EditableTableModel.SAVE_ALL) {
			...
			if ((model.saveconfig & EditableTableModel.SAVE_COLS) != 0)
				conf.append(model.getColsConfig());
			if ((model.saveconfig & EditableTableModel.SAVE_DATA) != 0)
				conf.append(model.getDataConfig());
			if ((model.saveconfig & EditableTableModel.SAVE_EDITABLE) != 0)
				conf.append(model.getEditableConfig());
			if ((model.saveconfig & EditableTableModel.SAVE_COLUMNS_SIZE) != 0)
				conf.append(getColumnsSizeConfig());
		}
