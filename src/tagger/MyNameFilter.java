...
public class MyNameFilter
{
	...
	if (file.getName().length()>maxNameLength)
	    {
		if (warnnamelength)
		    file.setError("FileNameLength","file name length "+file.getName().length()+" , superior limit was "+maxNameLength+";");
		file.match=false;
	    }
	else if (file.getName().length()<minNameLength)
	    {
		if (warnnamelength)
		    file.setError("FileNameLength","file name length "+file.getName().length()+" , inferior limit was "+minNameLength+";");
		file.match=false;
	    }
	if (equal.length()>0)
	    {
	...
	if (contains.size()>0)
		...
		for (i=0;i<contains.size();i++)
			...
			if (file.getName().indexOf((String)contains.get(i))==-1)
			    {
				if (warncontains)
				    file.setError("FileNameDoesNotContain","file name does not contain "+(String)contains.get(i)+";");
				file.match=false;
			    }
		    }
	    }
