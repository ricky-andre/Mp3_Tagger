...
public class UtilsWindow extends JFrame implements TreeModelListener
{
	...
	MyJTable table;
	// outputlistmode[0]=inside dir,outputlistmode[1]=dir level,outputlistmode[2]=root level
	...
	WinampWindow ()
	{
	    ...
	    grid.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),"Path for output lists",TitledBorder.LEFT,TitledBorder.TOP));
	    ...
	    //tmp2=createRadioPanel(outputlistmode[1],"on the same level of the directory"));
	    //tmp2.setBorder(BorderFactory.createEmptyBorder(0,5,0,0));
	    ...
	    grid=new JPanel();
	    grid.setLayout(new BoxLayout(grid,BoxLayout.Y_AXIS));
	    grid.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),"List write mode",TitledBorder.LEFT,TitledBorder.TOP));
	...
	public void actionPerformed (ActionEvent e)
	{
	    ...
	    if (command.equals("execute"))
		{
		    ...
		    else
			{
			    ...
			    JOptionPane.showMessageDialog(null,"Active process finishing ...","Active process finishing ...",JOptionPane.INFORMATION_MESSAGE);
			}
		}
	    else
		{
	...
	public boolean canExecute (String processId)
	{
	    ...
	    if (hastocheck)
		{
		    ...
		    if (!file.exists())
			{
			    taskOutput.addline(WarnPanel.ERROR,"<html><font color=black> Output directory&nbsp<font color=blue>\""+allpath+"\"</font> does not exists!");
			    refresh=true;
			    perform=false;
			}
	...
	public boolean taskExecute (String processId)
	{
	    ...
	    for (int i=0;i<paths.length;i++)
		{
		    ...
		    if (dir.outputlistpath.trim().length()>0)
			{
				...
				{
				    error=true;
				    taskOutput.addline(WarnPanel.ERROR,"<html><font color=black> Output directory&nbsp<font color=blue>\""+dir.outputlistpath+"\"</font> does not exists! Correct or remove dir!");
				}
			    else if (!file.isDirectory())
				{
				    error=true;
				    taskOutput.addline(WarnPanel.ERROR,"<html><font color=black> Output directory&nbsp<font color=blue>\""+dir.outputlistpath+"\"</font> is not a directory!");
				}
			}
		    if (!error)
			{
			    ...
			    for (int j=0;j<tempgetfiles.size();j++)
				{
					...
					{
					    /*label=new JLabel("<html><font size=-1 color=black> file <font color=blue>\""+tmpmyfile.getName()+"\"</font> not inserted, no mp3 extension!",danger,SwingConstants.LEFT);
					      label.setBackground(Color.white);
			    ...
			    label=new JLabel("<html><font size=-1 color=black> inserted <font color=blue>"+dir.files.size()+"</font> files",insfiles,SwingConstants.LEFT);
			    label.setBackground(Color.white);
			    label.setBorder(BorderFactory.createEmptyBorder(2,40,2,0));
			    taskOutput.addline(label);
		    ...
		    // add the directory even if it has no files inside or the directory does not exist!
		    filelist.add(dir);
		    // current++;
		    // progressmonitor.setProgress(current);
		}
	    ...
	    if (perform)
		{
		    ...
		    while (current<tasklength)
			{
			    ...
			    for (int i=0;i<filelist.size();i++)
				{
				    ...
				    for (int j=0;j<dir.files.size();j++)
					{
					    ...
					    else if (pathmode[0].isSelected()) // equals("relative"))
						{
						    try
							{
							    ...
							    if (ind!=-1)
								{
								    String relpath=fullfilepath.substring(ind+allpath.length(),fullfilepath.length());
								    list.append(relpath+"\n");
								}
							    else
				    ...
				    try
					{
					    if (dir.files.size()>0)
						{
						    ...
						    OutputStream outlistfile=new FileOutputStream (dir.outputlistpath+dir.outputlistname);
						    if (listmode[1].isSelected())
				    ...
				    catch (Exception e)
					{
					    ...
					    taskOutput.addline(WarnPanel.ERROR,"File <font color=blue>\""+dir.outputlistpath+dir.outputlistname+"\"</font> already exists and is read-only or is used by another application, can't write list!");
					}
				}
			    if (finished)
				break;
			    if (generateall && filelist.size()>0)
				{
				    ...
				    // RandomAccessFile outlistallfile=new RandomAccessFile(dir.outputlistpath+"/"+dir.outputlistname,"w");
    ...
    private class DatabaseWindow extends JPanel implements ActionListener, TreeSelectionListener, DocumentListener, TableModelListener, ItemListener, TaskExecuter
    {
	...
	// I use a hash to associate a command string with its button, and a hash to identify
	...
	public void actionPerformed (ActionEvent e)
	{
	    ...
	    if (command.equals("execute"))
		{
		    ...
		    else if (!taskActive)
			{
				...
				JOptionPane.showMessageDialog(null,"Close advanced search window!","Warning message",JOptionPane.WARNING_MESSAGE);
			}
		    else
			{
			    // warning window, waits for the task to finish!
			    JOptionPane.showMessageDialog(null,"Active process finishing ...","Warning message",JOptionPane.WARNING_MESSAGE);
			}
		}
	    ...
	    else
		{
		    ...
		    else if (command.equals("user field"))
			{
				...
				JOptionPane.showMessageDialog(null,"Cannot add another user field!","Error message",JOptionPane.ERROR_MESSAGE);
			}
	...
	public class advancedSearch extends JFrame
	{
		...
		advFieldSearchPanel (String str,int ind)
		{
		    ...
		    tmp.add(apply);
		    tmp.add(new JLabel("<html><font size=-1>apply&nbsp;<font color=blue><b>"+field+"</b></font>&nbsp;advanced&nbsp;search"));
		    mainPanel.add(tmp);
		    tmp=new JPanel();
		    tmp.setLayout (new BoxLayout(tmp,BoxLayout.X_AXIS));
		    tmp.setAlignmentX(JComponent.LEFT_ALIGNMENT);
		    ...
		    tooltip=new String [] {"move up selected rows","move down selected rows","add field of no interest","remove selected fields"};
	...
	public boolean canExecute (String processId)
	{
	    ...
	    if (paths.length==0 && perform)
		{
			...
			{
			    perform=false;
			    JOptionPane.showMessageDialog(null,"Select any directory in the tree or change\nthe output file to an existing one\nto perform ordering operation!","Error message",JOptionPane.INFORMATION_MESSAGE);
			}
		    else if (file.exists())
			{
				...
				{
				    perform=false;
				    JOptionPane.showMessageDialog(null,"The selected output file\n\n"+Utils.getCanonicalPath(file)+"\n\nis a directory! It must be a file!","Error message",JOptionPane.INFORMATION_MESSAGE);
				}
			}
		    else if (!orderoutput.isSelected())
			{
			    perform=false;
			    JOptionPane.showMessageDialog(null,"Select any directory in the tree!","Error message",JOptionPane.INFORMATION_MESSAGE);
			}
		}
	    else if (perform)
		{
			...
			{
			    perform=false;
			    JOptionPane.showMessageDialog(null,error.toString(),"Error, wrong fields selected!",JOptionPane.INFORMATION_MESSAGE);
			}
		}
	...
	public boolean taskExecute (String processId)
	{
	    ...
	    label=new JLabel("<html><font color=black><B>Inspecting Database properties ...",Utils.getImage("warnpanel","warninfo"),SwingConstants.LEFT);
	    label.setBackground(Color.white);
	    ...
	    while (current<tasklength && !finished)
		{
		    ...
		    for (int i=0;i<filelist.size();i++)
			{
			    ...
			    for (int j=0;j<dir.files.size();j++)
				{
				    ...
				    for (int k=0;k<fields.length;k++)
					{
					    ...
					    if (taghash.containsKey(fields[k]))
						{
							...
							{
							    seldbdata[current+1][k]=seldbdata[current+1][k].substring(4,seldbdata[current+1][k].length());
							}
						}
			    ...
			    if (generateall && dir.files.size()>0)
				{
				    ...
				    try
					{
					    OutputStream outlistfile=new FileOutputStream (dir.outputlistpath+File.separator+dir.outputlistname);
					    if (orderlist.length>0)
						{
					...
					{
					    System.out.println(e);
					    taskOutput.addline(WarnPanel.ERROR,"File "+dir.outputlistpath+File.separator+dir.outputlistname+" already exists and is read-only or is used by another application, can't write list!");
					}
				}
			}
		    if (seldbdata!=null)
		    ...
		    if (dbfile.exists() && newdbexists)
			{
			    ...
			    label=gimmeLabel();
			    label.setText("<html><font size=-1 color=black>Database "+Utils.toHtml("\""+name+"\"",Color.blue)+" found&nbsp;...");
			    taskOutput.addline(label);
			    try
				{
				    // read the new format and create a string that is
				    ...
				    label=gimmeLabel();
				    label.setText("<html><font size=-1 color=black>old Database column format: "+Utils.toHtml("\""+Utils.join(frstfld,",")+"\"",Color.blue));
				    taskOutput.addline(label);
				    label=gimmeLabel();
				    label.setText("<html><font size=-1 color=black>selected Database column format: "+Utils.toHtml("\""+Utils.join(db2.getColumns(),",")+"\"",Color.blue));
				    taskOutput.addline(label);
				    ...
				    label=gimmeLabel();
				    label.setText("<html><font size=-1 color=black>new Database column format: "+Utils.toHtml("\""+Utils.join(db.getColumns(),",")+"\"",Color.blue));
				    taskOutput.addline(label);
				...
				{
				    e.printStackTrace();
				    taskOutput.addline(WarnPanel.ERROR,"Error "+allpath+File.separator+name+" reading Database output file!");
				    current=tasklength;
				}
			}
		    ...
		    else
			{
			    db=db2;
			    label=gimmeLabel();
			    label.setText("<html><font size=-1 color=black>Database "+Utils.toHtml("\""+name+"\"",Color.blue)+" not found, creating a new one ...");
			    taskOutput.addline(label);
	...
	OrganizerWindow ()
	{
	    ...
	    casetable.setColumnEditor(1,new String[] {"","Large Case","UPPER CASE","lower case","First capitalized"});
	    ...
	    // then add the dirselection checkbox and the try button to perform the operation!
	    tmp4=new JPanel();
	    ...
	    filemode.add(copyrename[0]); filemode.add(copyrename[1]);
	...
	public boolean canExecute (String processId)
	{
	    ...
	    if (paths.length==0)
		{
		    JOptionPane.showMessageDialog(null,"Select any directory in the tree!","Error message",JOptionPane.ERROR_MESSAGE);
		    return false;
		}
	...
	public boolean taskExecute (String processId)
	{
	    ...
	    else if (processId.equals("organizertry"))
		{
		    ...
		    while (current<tasklength)
			{
			    ...
			    label=new JLabel("<html> "); label.setBackground(Color.white);
			    label.setBorder(BorderFactory.createEmptyBorder(10,0,10,0)); taskOutput.addline(label);
				    ...
				    for (int j=0;j<dir.files.size();j++)
					{
					    ...
					    for (int m=0;m<allfolders.length;m++)
						{
						    ...
						    for (int n=0;n<allfolders[m].length;n++)
							{
							    ...
							    tmpsubfolder=Utils.replace(tmpsubfolder,"< "+allfolders[m][n]+" >",fieldvalue);
							}
			    ...
			    // now add the JTree to the output panel and also the two "execute" and "try" buttons
    ...
    public class DoublesWindow extends JPanel implements ActionListener, ItemListener, TableModelListener, TaskExecuter
    {
	private JCheckBox recursesubdirs=new JCheckBox();
	private JCheckBox applyadvsearch=new JCheckBox();
	...
	// table with the duplicated files, there are buttons that this window will listen to
	...
	DoublesWindow ()
	{
	    ...
	    String tooltips[]=new String [] {"add another Database file to find duplicates","remove selected files from table"};
	    tmp3=new JPanel();
	    tmp3.setAlignmentX(JComponent.LEFT_ALIGNMENT);
	    tmp3.setLayout (new BoxLayout(tmp3,BoxLayout.X_AXIS));
	    ...
	    tmp5.setLayout (new BoxLayout(tmp5,BoxLayout.X_AXIS));
	    tmp5.setBorder(BorderFactory.createEmptyBorder(0,5,0,5));
            button=new MyButton (MyButton.NORMAL_BUTTON,null,"browseoutfile",Utils.getImage("main","browsedir"),this);
            button.setToolTipText("Browse directories");
	    tmp5.add(button);
            tmp3.add(tmp5);
	    tmp4.add(tmp3);
	    ...
	    tmp2.setAlignmentX (JComponent.LEFT_ALIGNMENT);
	    tmp2.setLayout (new BoxLayout(tmp2,BoxLayout.Y_AXIS));
	    tablemodel=new DinamicTableModel(new String[] {"CD / directory name","capacity (MB)","browse","filled (percent)"});
	    ...
	    tablemodel.setSaveConfig(tablemodel.SAVE_DATA);
	    containerslist.setToolTipText(browseindex,"click on the browse button to select an existing directory you want to fill!");
	    ...
	    tmp3.setLayout (new BoxLayout(tmp3,BoxLayout.X_AXIS));
	    tmp3.setBorder(BorderFactory.createEmptyBorder(0,5,5,5));
            button=new MyButton (MyButton.NORMAL_BUTTON,null,"browsecreatepath",Utils.getImage("main","browsedir"),this);
                    ...
                    if (filecontainers.size()>row)
                      {
                          ...
                          if (total!=0)
                                containerslist.setValueAt(new Integer((int)(filled/total*100)),row,filledindex);
                      }
                }
                            ...
                            if (file.exists() && file.isDirectory())
                            {
                            ...
                            else
                              {
                                  ...
                                  if (file.exists() && file.isDirectory())
                                      ...
                                      containerslist.setValueAt(new Integer(0),row,filledindex);
                              }
	...
	public void itemStateChanged (ItemEvent ie)
	{
	    ...
	    if (!elem.equals("") && ie.getStateChange()==ItemEvent.SELECTED)
		{
		    ...
		    if (source.equals(containersnum))
			{
			    try
				{
					...
					while (oldnum<num)
						...
						model.setValueAt(value,oldnum,capindex);
                                                model.setValueAt(new Integer(0),oldnum,model.getColumnIndex("filled (percent)"));
                                                button=new MyButton(MyButton.NORMAL_BUTTON,null,null,Utils.getImage("main","browsedir"),null);
                                                button.setBackground(Color.white);
				...
				{
                                    containerslist.addTableModelListener(this);
				    JOptionPane.showMessageDialog(null,"Wrong row number selected!","Wrong row number selected!",JOptionPane.ERROR_MESSAGE);
				}
			}
		    else if (source.equals(containerallequalto))
			{
				...
				{
				    JOptionPane.showMessageDialog(null,"Wrong number inserted!","Error message",JOptionPane.ERROR_MESSAGE);
	...
	public void actionPerformed (ActionEvent e)
	{
			    ...
			    int ind=model.getLastRowIndex();
			          ...
			          tabledbfiles.setValueAt(name,ind,nameindex);
                            ...
                            else
                                {
                                    ...
                                    JOptionPane.showMessageDialog(null,message,"Warning message",JOptionPane.INFORMATION_MESSAGE);
                                }
			}
		}
	    else if (command.equals("setcapacity"))
		{
			...
			{
			    JOptionPane.showMessageDialog(null,"Select at least a row!","Error message",JOptionPane.INFORMATION_MESSAGE);
			}
		}
	    ...
	    else if (command.equals("removefile"))
		{
		    ...
		    int max = lsm.getMaxSelectionIndex();
		        ...
		        ((DinamicTableModel)tabledbfiles.getModel()).removeRows(min,max);
                        ...
                        JOptionPane.showMessageDialog(null,"No Database selected in the table!","Warning message!",JOptionPane.INFORMATION_MESSAGE);
		}
	    ...
	    else if (command.startsWith("deletefile"))
		{
		    ...
		    if (n==0)
			{
			    ...
			    if (!file.exists())
				{
				    JOptionPane.showMessageDialog(null,"File does not exists, cannot remove!","Error message",JOptionPane.ERROR_MESSAGE);
				}
			    else if (!file.canWrite())
				{
				    JOptionPane.showMessageDialog(null,"File is read-only, cannot remove!","Error message",JOptionPane.ERROR_MESSAGE);
				}
			    else
				{
				    file.delete();
	...
	public boolean canExecute (String process)
	{
	    ...
	    else if (process.equals("doublesfind") && perform)
		{
		    ...
		    for (int i=0;i<model.data.size();i++)
			{
			    ...
			    if (path.trim().length()!=0 && name.trim().length()!=0)
				{
				    ...
				    if (!db1.checkHeader())
					{
					    ...
					    taskOutput.addline(WarnPanel.ERROR);
					    JOptionPane.showMessageDialog(null,"The first line of the Database file:\n\n"+path+name+"\n\nis corrupted, look the info panel for other info!","Database corrupted!",JOptionPane.INFORMATION_MESSAGE);
					}
				    else
					existone=true;
				}
			...
			{
			    perform=false;
			    JOptionPane.showMessageDialog(null,"Select one or more Database files\nin which duplicated songs have to be found!","Error, wrong configurations!",JOptionPane.INFORMATION_MESSAGE);
			}
		    else if (!existone)
			{
			    // check if doubled songs have to be searched also inside the Databases!
			    if (!doubleswindow.searchinside.isSelected())
				JOptionPane.showMessageDialog(null,"Select the \"search inside\" checkbox\nor select a Database file in which duplicated songs have to be searched!","Error, wrong configuration!",JOptionPane.INFORMATION_MESSAGE);
			}
		    if (perform && doubleswindow.savedoublelist.isSelected())
			{
				...
				{
				    perform=false;
				    JOptionPane.showMessageDialog(null,"The directory \""+file.getParent()+"\" does not exist,\nselect another existing directory for the output file!","Error, wrong configuration!",JOptionPane.INFORMATION_MESSAGE);
				}
			    else if (file.isDirectory())
				{
				    JOptionPane.showMessageDialog(null,"The output file name \""+file.getParent()+"\" is a directory,\nadd the file name to the directory, for example:\n\n"+"\""+file.getParent()+"myDatabase.txt\"","Error, wrong configuration!",JOptionPane.INFORMATION_MESSAGE);
				}
			}
		}
	    return perform;
	...
	*/
	...
	// it is used to indicate where the current, tasklength and statmessage has to be read!
	String taskFlag="";
	public boolean taskExecute (String processId)
	{
	    taskFlag="";
                                ...
                                else if (path.indexOf(selpath)==0)
                                    {
                                        ...
                                        if (occ2>occ1 && recursesubdirs.isSelected())
					                      ...
					                      +path+"\"</font> is a subdirectory of selected dir <font color=blue>\""
                                                              +file.getName()+"\"</font>. Select another container, move the selected dir or de-select recursion!");
                                              label.setBackground(Color.white);
			                      label.setBorder(BorderFactory.createEmptyBorder(2,0,2,0));
                                              taskOutput.addline(WarnPanel.ERROR,label);
                                          }
	    ...
	    if (!error && processId.equals("mulknap"))
		{
		    ...
		    for (int i=0;i<containerslist.getRowCount();i++)
			{
			    ...
			    } catch (Exception e) {capacity=(float)700.0;}
			    if (!file.exists())
				{
		    ...
		    for (int i=0;i<cont.length;i++)
			{
			    ...
			    for (int j=0;j<object.length;j++)
				{
				    ...
				    label=new JLabel("<html><font color=black>folder <font color=blue>\""+
						     ...
						     "<font color=blue>"+((int)file.getWeight())+"</font> MB",insfiles,SwingConstants.LEFT);
				    label.setBackground(Color.white);
			    ...
			    label.setBorder(BorderFactory.createEmptyBorder(10,40,5,0));
			    taskOutput.addline(label);
			    label=new JLabel("<html><font color=black>Moving listed dirs will fill this directory to <font color=blue>"+
	    ...
	    else if (!error && processId.equals("doublesfind"))
		{
		    ...
		    while (current<tasklength)
			{
			    ...
			    if (totalfiles>0)
				{
				    ...
				    progressmonitor.setNote(statMessage);
				    String dbfields[]=new String[] {"artist","title","album","bit rate","file name (all path)"};
				    String db[][]=new String[totalfiles+1][dbfields.length];
				    db[0][0]="artist"; db[0][1]="title"; db[0][2]="album";
				    db[0][3]="bit rate"; db[0][4]="file name (all path)";
				    int count=1;
				    for (int i=0;i<filelist.size();i++)
					{
					    dir=(selectedDir)(filelist.get(i));
			    ...
			    String tablefields[]=new String[] {"artist","title","album","delete file","source","bit rate","file name (all path if exists)"};
			    ...
			    // doublestable.setEditableColumn(doublestable.getColumnIndex("delete file"),true);
			    ...
			    // doublestable.setColumnClass(doublestable.getColumnIndex("delete file"),MyButton.class);
			    // fixColumns();
				    ...
				    for (int j=1;j<Databases.size();j++)
					{
					    ...
					    for (int k=0;k<db1.getRowCount();k++)
						{
						    ...
						    if (n!=-1)
							{
							    ...
							    // tablefields[]=new String[] {"artist","title","source","bit rate","file name","row"};
							    ...
							    for (int l=0;l<dbarr.length;l++)
								{
									...
									{
									    // discriminate between the fields, if the absolute path is empty get the simple path, if the absolute path exists, put the delete button to cancel the song!
									    if (tablefields[m].equals("delete file"))
										{
										    String fname=dbarr[l].getRowField(dbrowind[l],"file name (all path)");
										    if (fname.length()==0)
										...
										{
										    tmptablefields[m]=dbarr[l].getRowField(dbrowind[l],"file name (all path)");
	...
	for (int i=0;i<rows.length;i++)
		...
		if (Utils.occurences(rows[i],fieldsep)!=(oldfmt.length-1))
			...
			label.setText("<html><font size=-1 color=black>The existing Database seems to be corrupted, corrupted line:");
			taskOutput.addline(label);
			taskOutput.addline(WarnPanel.ERROR,label);
			label=gimmeLabel();
	...
	for (int i=0;i<outfmt.length;i++)
		...
		for (int j=0;j<oldfmt.length;j++)
			...
			{
			    //System.out.println("changing col "+i+" "+oldfmt[i]+"  and col "+j+" "+oldfmt[j]);
	...
	for (int i=0;i<rowcounter;i++)
	    ...
	    treemap.put(values,new Integer(i));
            current++;
            // statMessage="Reordererd "+current+" of "+tasklength;
        }
	...
	if (databasewindow.fieldseparator.getText().trim().length()==0)
        ...
        if (valuex!=null)
        {
            try {jtabbed.setSelectedIndex(valuex.intValue());}
            catch (Exception e) {};
        }

	databasewindow.table.setValueAt("every selected dir",0,databasewindow.getCol("directory"));
        databasewindow.table.setValueAt(config.getConfigString("5.2.dbpath"),0,databasewindow.getCol("output file dir"));
