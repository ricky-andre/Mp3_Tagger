...
{
    private static Hashtable inserted=new Hashtable ();
    ...
    private static Hashtable combos=new Hashtable();

    public static void addListener (ItemListener sourcecomp,JComboBox elem)
    {
    ...
    public static void removeListener (JComboBox elem)
    {
	combos.remove(elem);
    }

    // the original panel receives the event, checks if the file exists and eventually
    ...
    public static void syncronize (JComboBox source,String path)
    {
		...
		try
        ...
        else
          {
              /*
                ...
                of the combo boxes!!
              */
              JComboBox dirselect=null;
	      ItemListener comp=null;
              Enumeration hash_keys=combos.keys();
              while (hash_keys.hasMoreElements())
	      {
		  dirselect=(JComboBox)hash_keys.nextElement();
