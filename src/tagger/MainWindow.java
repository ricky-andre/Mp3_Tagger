...
public class MainWindow extends JFrame implements ActionListener
{
	...
	*/
	...
	// selectInfo.setText("<html><font size=-1><B>Selected items:&nbsp;<font color=blue>"+0+"/"+counter);
	...
	filteredList=new GetFiles(dir,rec,config.optionwincfg.fileFilter,GetFiles.KEEP_WRONG_FILES,dir_tree.getRoot());
	...
	for (int i=0;i<filteredList.size();i++)
		...
		try
			...
			if (radioButtonPathType[0].isSelected())
			    {
				fileListData[i][0]=Utils.replaceAll(tmp.getCanonicalPath(),Utils.pathseparator," "+Utils.pathseparator+" ");
			    }
			else if (radioButtonPathType[1].isSelected())
			    {
				//fileListData[i][0]=Utils.replaceAll(tmp.rel_path.substring(filteredList.rootPath.length()+1,tmp.rel_path.length()),Utils.pathseparator," "+Utils.pathseparator+" ");
                                fileListData[i][0]=Utils.replaceAll(tmp.rel_path+File.separator+tmp.getName(),File.separator," "+File.separator+" ");
			    }
			else if (radioButtonPathType[2].isSelected())
			    {
	...
	// selectInfo.setText("<html><font size=-1><B>Selected items:&nbsp;<font color=blue>"+filteredList.size()+"/"+filteredList.size());
	...
	for (int i=0;i<maintitles.length;i++)
		...
		// button.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEmptyBorder(),maintitles[i],TitledBorder.CENTER,TitledBorder.BELOW_BOTTOM,new Font("Serif",Font.PLAIN,14),Color.black));
		//JLabel title=new JLabel("<html><font color=black>"+maintitles[i]+"</html>",SwingConstants.CENTER);
	...
	tmppanel.setBorder(BorderFactory.createTitledBorder(bord,"Selected directories",TitledBorder.LEFT,TitledBorder.TOP));
	tmp=new JPanel();
	tmp.setLayout(new BoxLayout(tmp,BoxLayout.Y_AXIS));
	...
	// dir_tree has to be inizialized BEFORE the fileTable, since the rootPath must be inizialized!
	...
	files_pane = new JScrollPane (fileTable,ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
	...
	textScrollArea=new JScrollPane(warningText,ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,ScrollPaneConstants.HORIZONTAL_SCROLLBAR_ALWAYS);
	...
	if (valuex!=null && valuex.intValue()!=0)
            ...
            public void windowClosing(WindowEvent e)
		{
		    ...
		    if (banner!=null)
                    ...
                    {
                          try
                          { Runtime.getRuntime().exec(config.getConfigString("Database.dbstop")); }
                          catch (Exception ex)
                          {}
                    }
		    System.exit(0);
       ...
       //System.out.println("main %"+(System.currentTimeMillis()-time)*100/20x7fffffff);
      ...
      */
       ...
       {
          try
          { Runtime.getRuntime().exec(config.getConfigString("Database.dbstart")); }
          catch (Exception e) {}
       }
       setVisible(true);
    }
	...
	if (command.equals("image0"))
		...
		if (taskActive)
		    ...
		    JOptionPane.showMessageDialog(null,"Close the opened window before\nopening a new one!","Information message",JOptionPane.INFORMATION_MESSAGE);
	    }
		...
		if (tagwindow!=null && windowOpen[2]==true)
		    ...
		    JOptionPane.showMessageDialog(null,"Open \"Tag Window\" or \"Rename Window\"\nbefore pressing this button!","Error message!",JOptionPane.INFORMATION_MESSAGE);
	    }
		...
		for (int i=0;i<size;i++)
			...
			try
			    {
				fileListData [i][0] = Utils.replaceAll((filteredList.getElem(i)).getCanonicalPath(),Utils.pathseparator," "+Utils.pathseparator+" ");
			    }
			catch (Exception ex)
			    {
				fileListData [i][0] = "";
		...
		// selectInfo.setText("<html><font size=-1><B>Selected items:&nbsp; <font color=blue>"+total+"/"+total);
		selectInfo.setText("Selected items: "+total+"/"+total);
	    }
          ...
          for (int i=0;i<dir.length;i++)
               {
                  if (!(new File(dir_tree.getRoot()+File.separator+dir[i]).exists()))
                      {
                          warningText.append("Directory <font color=blue>\""+dir[i]+"\"</font> has been deleted, moved or renamed!");
	...
	scanDirMonitor (String command)
	{
	    ...
	    // progressMonitor = new ProgressMonitor(this,"Loading file info","", 0, task.getLengthOfTask());
