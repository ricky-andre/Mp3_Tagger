...
public abstract class Utils {
	...
	public static String[][] findMatch(String orig, String match) {
		...
		while (currentMatch < tmp.length()) {
			...
			// System.out.println("first '<' pos "+nfs+" first '>' pos "+nfe+" field
			// "+tmp.substring(nfs+2,nfe));

			nfs = tmp.indexOf("< ", currentMatch + nfe - nfs);
			if (nfs != -1) {
				String key = new String(tmp.substring(nfe + 2, nfs));
				int cut = tmporig.indexOf(key);
				// System.out.println("key to find in original string "+key+" found at pos
				// "+cut);
				if (cut == -1)
					return null;
				else {
				...
				if (currentMatch < tmp.length()) {
					// System.out.println("posizione di curr match "+currentMatch+" nfe+2
					// "+(nfe+2));
	...
	public static void printMessage(String str[], int code) {
		if (code == 0) {
			// JOptionPane.showMessageDialog(null,"Cannot perform operation, you have
			// already\nadded "+str[0]+" files, limit was
			// "+str[1]+"!",JOptionPane.ERROR_MESSAGE);
	...
	private static int spfunction1[] = new int[] { 0x1010400, 0, 0x10000, 0x1010404, 0x1010004, 0x10404, 0x4, 0x10000,
			0x400, 0x1010400, 0x1010404, 0x400, 0x1000404, 0x1010004, 0x1000000, 0x4, 0x404, 0x1000400, 0x1000400,
			0x10400, 0x10400, 0x1010000, 0x1010000, 0x1000404, 0x10004, 0x1000004, 0x1000004, 0x10004, 0, 0x404,
			0x10404, 0x1000000, 0x10000, 0x1010404, 0x4, 0x1010000, 0x1010400, 0x1000000, 0x1000000, 0x400, 0x1010004,
			0x10000, 0x10400, 0x1000004, 0x400, 0x4, 0x1000404, 0x10404, 0x1010404, 0x10004, 0x1010000, 0x1000404,
			0x1000004, 0x404, 0x10404, 0x1010400, 0x404, 0x1000400, 0x1000400, 0, 0x10004, 0x10400, 0, 0x1010004 };
	private static int spfunction2[] = new int[] { 0x80108020, 0x80008000, 0x8000, 0x108020, 0x100000, 0x20, 0x80100020,
			0x80008020, 0x80000020, 0x80108020, 0x80108000, 0x80000000, 0x80008000, 0x100000, 0x20, 0x80100020,
			0x108000, 0x100020, 0x80008020, 0, 0x80000000, 0x8000, 0x108020, 0x80100000, 0x100020, 0x80000020, 0,
			0x108000, 0x8020, 0x80108000, 0x80100000, 0x8020, 0, 0x108020, 0x80100020, 0x100000, 0x80008020, 0x80100000,
			0x80108000, 0x8000, 0x80100000, 0x80008000, 0x20, 0x80108020, 0x108020, 0x20, 0x8000, 0x80000000, 0x8020,
			0x80108000, 0x100000, 0x80000020, 0x100020, 0x80008020, 0x80000020, 0x100020, 0x108000, 0, 0x80008000,
			0x8020, 0x80000000, 0x80100020, 0x80108020, 0x108000 };
	private static int spfunction3[] = new int[] { 0x208, 0x8020200, 0, 0x8020008, 0x8000200, 0, 0x20208, 0x8000200,
			0x20008, 0x8000008, 0x8000008, 0x20000, 0x8020208, 0x20008, 0x8020000, 0x208, 0x8000000, 0x8, 0x8020200,
			0x200, 0x20200, 0x8020000, 0x8020008, 0x20208, 0x8000208, 0x20200, 0x20000, 0x8000208, 0x8, 0x8020208,
			0x200, 0x8000000, 0x8020200, 0x8000000, 0x20008, 0x208, 0x20000, 0x8020200, 0x8000200, 0, 0x200, 0x20008,
			0x8020208, 0x8000200, 0x8000008, 0x200, 0, 0x8020008, 0x8000208, 0x20000, 0x8000000, 0x8020208, 0x8,
			0x20208, 0x20200, 0x8000008, 0x8020000, 0x8000208, 0x208, 0x8020000, 0x20208, 0x8, 0x8020008, 0x20200 };
	private static int spfunction4[] = new int[] { 0x802001, 0x2081, 0x2081, 0x80, 0x802080, 0x800081, 0x800001, 0x2001,
			0, 0x802000, 0x802000, 0x802081, 0x81, 0, 0x800080, 0x800001, 0x1, 0x2000, 0x800000, 0x802001, 0x80,
			0x800000, 0x2001, 0x2080, 0x800081, 0x1, 0x2080, 0x800080, 0x2000, 0x802080, 0x802081, 0x81, 0x800080,
			0x800001, 0x802000, 0x802081, 0x81, 0, 0, 0x802000, 0x2080, 0x800080, 0x800081, 0x1, 0x802001, 0x2081,
			0x2081, 0x80, 0x802081, 0x81, 0x1, 0x2000, 0x800001, 0x2001, 0x802080, 0x800081, 0x2001, 0x2080, 0x800000,
			0x802001, 0x80, 0x800000, 0x2000, 0x802080 };
	private static int spfunction5[] = new int[] { 0x100, 0x2080100, 0x2080000, 0x42000100, 0x80000, 0x100, 0x40000000,
			0x2080000, 0x40080100, 0x80000, 0x2000100, 0x40080100, 0x42000100, 0x42080000, 0x80100, 0x40000000,
			0x2000000, 0x40080000, 0x40080000, 0, 0x40000100, 0x42080100, 0x42080100, 0x2000100, 0x42080000, 0x40000100,
			0, 0x42000000, 0x2080100, 0x2000000, 0x42000000, 0x80100, 0x80000, 0x42000100, 0x100, 0x2000000, 0x40000000,
	...
	private static int spfunction6[] = new int[] { 0x20000010, 0x20400000, 0x4000, 0x20404010, 0x20400000, 0x10,
			0x20404010, 0x400000, 0x20004000, 0x404010, 0x400000, 0x20000010, 0x400010, 0x20004000, 0x20000000, 0x4010,
			0, 0x400010, 0x20004010, 0x4000, 0x404000, 0x20004010, 0x10, 0x20400010, 0x20400010, 0, 0x404010,
			0x20404000, 0x4010, 0x404000, 0x20404000, 0x20000000, 0x20004000, 0x10, 0x20400010, 0x404000, 0x20404010,
			0x400000, 0x4010, 0x20000010, 0x400000, 0x20004000, 0x20000000, 0x4010, 0x20000010, 0x20404010, 0x404000,
			0x20400000, 0x404010, 0x20404000, 0, 0x20400010, 0x10, 0x4000, 0x20400000, 0x404010, 0x4000, 0x400010,
			0x20004010, 0, 0x20404000, 0x20000000, 0x400010, 0x20004010 };
	private static int spfunction7[] = new int[] { 0x200000, 0x4200002, 0x4000802, 0, 0x800, 0x4000802, 0x200802,
			0x4200800, 0x4200802, 0x200000, 0, 0x4000002, 0x2, 0x4000000, 0x4200002, 0x802, 0x4000800, 0x200802,
			0x200002, 0x4000800, 0x4000002, 0x4200000, 0x4200800, 0x200002, 0x4200000, 0x800, 0x802, 0x4200802,
			0x200800, 0x2, 0x4000000, 0x200800, 0x4000000, 0x200800, 0x200000, 0x4000802, 0x4000802, 0x4200002,
			0x4200002, 0x2, 0x200002, 0x4000000, 0x4000800, 0x200000, 0x4200800, 0x802, 0x200802, 0x4200800, 0x802,
			0x4000002, 0x4200802, 0x4200000, 0x200800, 0, 0x2, 0x4200802, 0, 0x200802, 0x4200000, 0x800, 0x4000002,
			0x4000800, 0x800, 0x200002 };
	private static int spfunction8[] = new int[] { 0x10001040, 0x1000, 0x40000, 0x10041040, 0x10000000, 0x10001040,
			0x40, 0x10000000, 0x40040, 0x10040000, 0x10041040, 0x41000, 0x10041000, 0x41040, 0x1000, 0x40, 0x10040000,
			0x10000040, 0x10001000, 0x1040, 0x41000, 0x40040, 0x10040040, 0x10041000, 0x1040, 0, 0, 0x10040040,
			0x10000040, 0x10001000, 0x41040, 0x40000, 0x41040, 0x40000, 0x10041000, 0x1000, 0x40, 0x10040040, 0x1000,
			0x41040, 0x10001000, 0x40, 0x10000040, 0x10040000, 0x10040040, 0x10000000, 0x40000, 0x10001040, 0,
			0x10041040, 0x40040, 0x10000040, 0x10040000, 0x10001000, 0x10001040, 0, 0x10041040, 0x41000, 0x41000,
			0x1040, 0x1040, 0x40040, 0x10000000, 0x10041000 };
	...
	private static Object desalghoritm(Object obj, int encrypt, String stringa) {
		...
		while (m < len) {
			...
			if (message != null) {
				left = (((int) (message.charAt(m++) & 0x00ff) << 24)) | (((int) (message.charAt(m++) & 0x00ff) << 16))
						| (((int) (message.charAt(m++) & 0x00ff) << 8)) | ((int) (message.charAt(m++) & 0x00ff));
				right = (((int) (message.charAt(m++) & 0x00ff) << 24)) | (((int) (message.charAt(m++) & 0x00ff) << 16))
						| (((int) (message.charAt(m++) & 0x00ff) << 8)) | ((int) (message.charAt(m++) & 0x00ff));
			} else if (bytes != null) {
				left = (((int) bytes[m++] & 0xff) << 24) | (((int) bytes[m++] & 0xff) << 16)
						| (((int) bytes[m++] & 0xff) << 8) | ((int) bytes[m++] & 0xff);
				right = (((int) bytes[m++] & 0xff) << 24) | (((int) bytes[m++] & 0xff) << 16)
						| (((int) bytes[m++] & 0xff) << 8) | ((int) bytes[m++] & 0xff);
			}
			 ...
			 * );
			 * System.out.println(" byte to char to int to string "+Integer.toBinaryString((
			 * int)((char)(b))));
			...
			if (message != null)
				tempresult.append(new String(new char[] { (char) (left >>> 24), (char) ((left >>> 16) & 0xff),
						(char) ((left >>> 8) & 0xff), (char) (left & 0xff), (char) (right >>> 24),
						(char) ((right >>> 16) & 0xff), (char) ((right >>> 8) & 0xff), (char) (right & 0xff) }));
			else if (bytes != null) {
				bresult[m - 8] = (byte) (left >>> 24);
				bresult[m - 7] = (byte) ((left >>> 16) & 0xff);
				bresult[m - 6] = (byte) ((left >>> 8) & 0xff);
				bresult[m - 5] = (byte) (left & 0xff);
				bresult[m - 4] = (byte) (right >>> 24);
				bresult[m - 3] = (byte) ((right >>> 16) & 0xff);
				bresult[m - 2] = (byte) ((right >>> 8) & 0xff);
				bresult[m - 1] = (byte) (right & 0xff);
			}
			 ...
			 * if (message!=null)
			 * {
			 * String res=new String(new char[] {(char)(left>>>24),(char)((left>>>16) &
			 * 0xff),(char)((left>>>8) & 0xff),(char)(left &
			 * 0xff),(char)(right>>>24),(char)((right>>>16) & 0xff),(char)((right>>>8) &
			 * 0xff),(char)(right & 0xff)});
	...
	private static int[] des_createKeys(String key) {
		int pc2bytes0[] = new int[] { 0, 0x4, 0x20000000, 0x20000004, 0x10000, 0x10004, 0x20010000, 0x20010004, 0x200,
				0x204, 0x20000200, 0x20000204, 0x10200, 0x10204, 0x20010200, 0x20010204 };
		int pc2bytes1[] = new int[] { 0, 0x1, 0x100000, 0x100001, 0x4000000, 0x4000001, 0x4100000, 0x4100001, 0x100,
				0x101, 0x100100, 0x100101, 0x4000100, 0x4000101, 0x4100100, 0x4100101 };
		int pc2bytes2[] = new int[] { 0, 0x8, 0x800, 0x808, 0x1000000, 0x1000008, 0x1000800, 0x1000808, 0, 0x8, 0x800,
				0x808, 0x1000000, 0x1000008, 0x1000800, 0x1000808 };
		int pc2bytes3[] = new int[] { 0, 0x200000, 0x8000000, 0x8200000, 0x2000, 0x202000, 0x8002000, 0x8202000,
				0x20000, 0x220000, 0x8020000, 0x8220000, 0x22000, 0x222000, 0x8022000, 0x8222000 };
		int pc2bytes4[] = new int[] { 0, 0x40000, 0x10, 0x40010, 0, 0x40000, 0x10, 0x40010, 0x1000, 0x41000, 0x1010,
				0x41010, 0x1000, 0x41000, 0x1010, 0x41010 };
		int pc2bytes5[] = new int[] { 0, 0x400, 0x20, 0x420, 0, 0x400, 0x20, 0x420, 0x2000000, 0x2000400, 0x2000020,
				0x2000420, 0x2000000, 0x2000400, 0x2000020, 0x2000420 };
		int pc2bytes6[] = new int[] { 0, 0x10000000, 0x80000, 0x10080000, 0x2, 0x10000002, 0x80002, 0x10080002, 0,
				0x10000000, 0x80000, 0x10080000, 0x2, 0x10000002, 0x80002, 0x10080002 };
		int pc2bytes7[] = new int[] { 0, 0x10000, 0x800, 0x10800, 0x20000000, 0x20010000, 0x20000800, 0x20010800,
				0x20000, 0x30000, 0x20800, 0x30800, 0x20020000, 0x20030000, 0x20020800, 0x20030800 };
		int pc2bytes8[] = new int[] { 0, 0x40000, 0, 0x40000, 0x2, 0x40002, 0x2, 0x40002, 0x2000000, 0x2040000,
				0x2000000, 0x2040000, 0x2000002, 0x2040002, 0x2000002, 0x2040002 };
		int pc2bytes9[] = new int[] { 0, 0x10000000, 0x8, 0x10000008, 0, 0x10000000, 0x8, 0x10000008, 0x400, 0x10000400,
				0x408, 0x10000408, 0x400, 0x10000400, 0x408, 0x10000408 };
		int pc2bytes10[] = new int[] { 0, 0x20, 0, 0x20, 0x100000, 0x100020, 0x100000, 0x100020, 0x2000, 0x2020, 0x2000,
				0x2020, 0x102000, 0x102020, 0x102000, 0x102020 };
		int pc2bytes11[] = new int[] { 0, 0x1000000, 0x200, 0x1000200, 0x200000, 0x1200000, 0x200200, 0x1200200,
				0x4000000, 0x5000000, 0x4000200, 0x5000200, 0x4200000, 0x5200000, 0x4200200, 0x5200200 };
		int pc2bytes12[] = new int[] { 0, 0x1000, 0x8000000, 0x8001000, 0x80000, 0x81000, 0x8080000, 0x8081000, 0x10,
				0x1010, 0x8000010, 0x8001010, 0x80010, 0x81010, 0x8080010, 0x8081010 };
		int pc2bytes13[] = new int[] { 0, 0x4, 0x100, 0x104, 0, 0x4, 0x100, 0x104, 0x1, 0x5, 0x101, 0x105, 0x1, 0x5,
				0x101, 0x105 };
		...
		boolean shifts[] = new boolean[] { false, false, true, true, true, true, true, true, false, true, true, true,
				true, true, true, false };
		// other variables
		int lefttemp = 0, righttemp = 0, m = 0, n = 0, temp = 0, left = 0, right = 0;

		for (int j = 0; j < iterations; j++) { // either 1 or 3 iterations
			left = ((int) (key.charAt(m++) & 0x00ff) << 24) | ((int) (key.charAt(m++) & 0x00ff) << 16)
					| ((int) (key.charAt(m++) & 0x00ff) << 8) | (int) (key.charAt(m++) & 0x00ff);

			right = ((int) (key.charAt(m++) & 0x00ff) << 24) | ((int) (key.charAt(m++) & 0x00ff) << 16)
					| ((int) (key.charAt(m++) & 0x00ff) << 8) | (int) (key.charAt(m++) & 0x00ff);
			...
			// the right side needs to be shifted and to get the last four bits of the left
			// side
			...
			for (int i = 0; i < shifts.length; i++) {
				...
				// this conversion will look like PC-2 except only the last 6 bits of each byte
				// are used
