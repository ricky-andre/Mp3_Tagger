...
class AdvancedTagWindow extends JFrame implements ActionListener, ItemListener
{
    ...
    private Hashtable saveconfig=null;
    ...
    private Hashtable panelshash=new Hashtable();
    private Hashtable iconshash=new Hashtable();
    ...
    AdvancedTagWindow (int mode,Hashtable cfgtosave,Mp3info mp3)
    {
	super ();
	mod=mode;
	file=mp3;
    ...
    AdvancedTagWindow (int mode,Hashtable cfgtosave)
    {
	super ();
	mod=mode;
	createWindow();
    ...
    /*
      ...
      {
      if (panelshash.containsKey(name))
        return (((Id3v2panel)panelshash.get(name)).getComponent());
	else
	    {
		System.out.println("invalid string in adv window"+name);
		return null;
	    }
    }
    */
	...
	mainPanel.setLayout(new BoxLayout(mainPanel,BoxLayout.Y_AXIS));
	mainPanel.setBorder(BorderFactory.createEmptyBorder(5,10,5,10));
	Id3v2LyricsPanel lyrpanel=new Id3v2LyricsPanel(Mp3info.getConfigObject("unsynchronized lyric/text transcription"));
	lyrpanel.setBorder(BorderFactory.createTitledBorder("Lyrics"));
	Id3v2panels.add(lyrpanel);
	icons.add(lyrpanel.getIconSelect());
	mainPanel.add(lyrpanel);
	panelshash.put("unsynchronized lyric/text transcription",lyrpanel);
        jtabbed.add("Lyrics",mainPanel);*/


	// add Database elements, other song info, dates and times ...
	/*
	  tmp=new String[] {
	...
	*/
     ...
     */
    public static void loadConfigHash (int mode, Hashtable saveconfig)
    {
	String prefix=null;
