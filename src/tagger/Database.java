...
public class Database implements DatabaseInterface
{
    ...
    // numbers of letters that have to match between the names to consider them valid!
    ...
    static
    {
	dontput.put("&","1"); dontput.put("and","1");
	dontput.put("ft.","1");	dontput.put("featuring","1");
    }
	...
	try
		...
		if (mem<filetoread.length()*3)
		    {
			// System.out.println("Available memory "+((float)mem/1000000)+" MB file len "+((float)filetoread.length()/1000000));
			JOptionPane.showMessageDialog(null,"You have not enough memory available. Close some applications\nor try to launch Mp3 Studio with -mx60m parameter!","Error message",JOptionPane.WARNING_MESSAGE);
			return false;
		    }
		else
		    {
	...
	if (data==null)
		...
		try
			...
			try
			    {filetoread.close();}
			catch (Exception ex) {System.out.println("Unable to close file");}
