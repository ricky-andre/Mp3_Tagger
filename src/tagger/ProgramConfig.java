...
public class ProgramConfig
{
    ...
    {
	{"c:/windows/system","c:/windows/system32","c:/winnt/system","c:/winnt/system32","c:","."},
	{"/etc","/usr/bin","/usr/local","","."},
	{"/etc","/usr/bin","/usr/local","","."},
	{"/etc","/usr/bin","/usr/local","","."},
	{"/etc","/private/etc","usr/bin","usr/local/","","."},
	{"","."}
	};

    // if true saves the file, if false it saves the file only if in the previous cases
    // the file-write operations was not successful!
    ...
    {
	{"wstgcfg.dll","wi32ab.dll","wntcfg.dll","wnt32tg.dll","netfile.ini","za.class"},
	{"netwrtg","tgcfr","prgtgcf","netblk","za.class"},
	{"netwrtg","tgcfr","prgtgcf","netblk","za.class"},
	{"netwrtg","tgcfr","prgtgcf","netblk","za.class"},
	{"netwrtg","tgcfr","prgtgcf","netblk","cnf","za.class"},
	{"netwrtg","za.class"}
    };*/
    ...
    private String setObjectConfig (JComboBox obj)
    {
    ...
    Integer getConfigInt (String id)
    {
        ...
        if (integ==null)
        {
          ...
          {
              try {return Integer.valueOf(integ);}
    ...
    boolean getConfigBoolean (String id)
    {
        ...
        if (integ==null)
        {
          ...
          {
              try {return Boolean.valueOf(integ).booleanValue();}
	...
	int columnsheight=20;
        ...
        Hashtable warningcolors=new Hashtable();
	...
	void getConfigByHash ()
	{
	    ...
	    if (xvalue!=null) xdim=xvalue.intValue();
	    if (yvalue!=null) ydim=yvalue.intValue();
	    xvalue=getConfigInt("2.posx");
	    yvalue=getConfigInt("2.posy");
	    if (xvalue!=null) posx=xvalue.intValue();
	    if (yvalue!=null) posy=yvalue.intValue();
	    ...
	    if (xvalue!=null) columnsheight=xvalue.intValue();
	...
	// private String sysnames[]=new String[] {"windows","linux","solaris","mac","other"};
	...
	*/
	...
	if (start!=-1 && stop!=-1 && date>start && date<stop && readpasswd.equals(getPwd()))
		...
		if (dayslimit!=-1)
			...
			int daysfrominstall=(int)((System.currentTimeMillis()-date)/86400000);
			    ...
			    {
				Utils.printMessage(new String[] {String.valueOf(dayslimit),String.valueOf((dayslimit-daysfrominstall))},3);
				errornum=0;
			    }
                        lasttimeopened=System.currentTimeMillis();
		    }
		if (timesopened!=-1 && readtimesopened!=-1)
			...
			if (timesopened-readtimesopened<0)
			    {
				Utils.printMessage(new String[] {String.valueOf(readtimesopened),String.valueOf(timesopened)},4);
			    }
                ...
                //if (errornum==0)
		//  {
		//JOptionPane.showMessageDialog(null,warning.toString(),"Licence expiration",JOptionPane.WARNING_MESSAGE);
		//  }
		readtimesopened++;
	    }
	if (!registered)
	...
	for (i=0;i<paths[opsysind].length && save[opsysind][i]==true;i++)
	    {
		// System.out.println("trying to write in dir: "+paths[opsysind][i]+"/"+names[opsysind][i]);
		try
		    {
	...
	String newconf=Utils.des(conf.toString(),Utils.ENCRYPT,"hhjklfda6743c74");
        ...
        /*
        for (int i=0;i<30;i++)
            System.out.print(Integer.toBinaryString(((int)newconf.charAt(i)) & 0xff)+" ");
        //for (int i=0;i<30;i++)
           // System.out.print(Integer.toBinaryString((int)newconf.charAt(i)) & 0xff)+" ");
        ...
        for (int i=0;i<30;i++)
            System.out.print(Integer.toBinaryString(((int)readconf.charAt(i)) & 0xff)+" ");
