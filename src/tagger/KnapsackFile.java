...
public class KnapsackFile extends MyFile implements KnapsackItem, ContainerItem, TaskExecuter {
    ...
    public boolean moveTo(ContainerItem cont) {
        ...
        try {
            ...
            if (scanlevel != RECURSEALL || mode == COPY_FILES) {
                ...
                TreeMap<Integer, File> dirstodelete = new TreeMap<Integer, File>();
                ...
                for (int i = 0; i < files.size() && !finished; i++) {
                    ...
                    dirstodelete.put(Integer.valueOf(-1 * tmp.rec_level),
                            new File(getAbsolutePath() + File.separator + tmp.rel_path));
                ...
                Set<Map.Entry<Integer, File>> set = dirstodelete.entrySet();
                Iterator<Map.Entry<Integer, File>> iter = set.iterator();
                while (iter.hasNext() && !finished) {
                    Map.Entry<Integer, File> item = (Map.Entry<Integer, File>) iter.next();
    ...
    public void clearErrors() {
        hasherror = new Hashtable<String, ArrayList<error>>();
    }

    private void setError(String errortype, error err) {
        ArrayList<error> tmp = null;
        if (!hasherror.containsKey(errortype))
            tmp = new ArrayList<error>();
        else
            tmp = (ArrayList<error>) hasherror.get(errortype);
        tmp.add(err);
    ...
    int getErrorSize(String errortype) {
        if (hasherror.containsKey(errortype))
            return ((ArrayList<error>) hasherror.get(errortype)).size();
        else
            return 0;
    }
