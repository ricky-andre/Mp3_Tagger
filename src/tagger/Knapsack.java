...
public class Knapsack implements TaskExecuter
{
    ...
    private class multipleSolution
    {
        ...
        public void fixLastContainer ()
        {
            ...
            unusedspace=left;
            if (debug>0)
                  System.out.println("Unused space "+unusedspace+" inserted "+itemsinserted+" of "+origitems.length);
    ...
    private class combinationScrambler
    {
        ...
        private class combinationNode
        {
            ...
            {
                inserted.remove(new Integer(ind));
                int i=ind+1;
                while (i<size && (inserted.containsKey(new Integer(i)) || i==index || i==ind))
                    i++;
                if (i>=size)
                    {
                        if (father!=null)
                            ...
                            {
                                inserted.remove(new Integer(index));
                                index++;
                                inserted.put(new Integer(index),"");
                                if (index<size)
            ...
            public void add ()
            {
                ...
                if (inserted.size()<size)
                    {
                        int i=0;
                        while ((inserted.containsKey(new Integer(i)) || i==index) && i<size)
                            i++;
                        if (i<size)
                          {
                            son=new combinationNode(i);
