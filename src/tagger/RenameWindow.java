...
public class RenameWindow extends JFrame implements ActionListener, ItemListener {
	...
	private boolean tableinsert(int i) {
		...
		if (!error && pos > -2) {
			...
			if (pos == -1)
				...
				warningArea.append(", cannot insert '" + str + "' at position " + pos + ", file name length is only "
						+ tmp.length());
				return false;
			}
			return true;
	...
	RenameWindow(MainWindow win, final int windowId) {
		...
		// (table,ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);

		warningArea = new WarnPanel();
		warningScrollPane = new JScrollPane(warningArea, ScrollPaneConstants.VERTICAL_SCROLLBAR_ALWAYS,
	...
	private void removeSuccess() {
		...
		// if try mode was used, or no file has been tagged, the selection will not be
		// removed!
	...
	private void doAction(String command) {
		if (command.equals("execute") || command.equals("try")) {
			...
			if (selFiles.length > 0 && sel.length > 0 && sel[0] < selFiles.length
					&& singlerename.getText().length() > 0) {
				int i = sel[0];
				...
				if (selFiles[i].canWrite()) {
					...
					JOptionPane.showMessageDialog(null, "Cannot rename, file is read only!", "Error message",
							JOptionPane.ERROR_MESSAGE);
	...
	public class MyProgressMonitor {
		...
		public void exec(String process, String command) {
			if (!taskActive) {
				...
				// calculate the number of selected files, to set the proper number in progress
				// window
				taskActive = true;
				...
				progressMonitor = new ProgressMonitor(warningwindow, "Renaming files ...", "", 0,
						task.getLengthOfTask());
				progressMonitor.setProgress(0);
				progressMonitor.setMillisToDecideToPopup(1000);
			} else {
				// display a warning window, renaming process active!
				JOptionPane.showMessageDialog(null, "Renaming process still active ...",
						"Renaming process still active ...", JOptionPane.INFORMATION_MESSAGE);
			}
		}
